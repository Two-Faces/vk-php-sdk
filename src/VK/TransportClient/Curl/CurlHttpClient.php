...
class CurlHttpClient implements TransportClient
{
	...
	{
		$payload = [];
		$payload[$parameter_name] = (class_exists('CURLFile', false)) ? new CURLFile($path) : '@' . $path;
	 ...
	 * @return TransportClientResponse
	 */
	protected function parseRawResponse(int $http_status, string $response): TransportClientResponse
	{
